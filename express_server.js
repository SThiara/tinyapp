...
// also see about making a separate page with a header to handle error messages
...
app.get("/u/:shortURL", (req, res) => {
  ...
  if (req.cookies.uniqueTracker === undefined) {
    uniqueVisitorID = generateRandomString();
    res.cookie("uniqueTracker", uniqueVisitorID);
  } else {
    uniqueVisitorID = req.cookies.uniqueTracker;
  };
  if (!(urlDatabase[req.params.shortURL].uniqueVisitors.includes(uniqueVisitorID))) {
    urlDatabase[req.params.shortURL].uniqueVisitors.push(uniqueVisitorID); // search URL's array to see if the ID has already been added; if not, add  
  };
  urlDatabase[req.params.shortURL].visitTimes.push({
